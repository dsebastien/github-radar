...
- **Noise controls**: all four hide toggles are on by default. "Hide drafts" drops draft pull requests. "Hide bots" drops items opened by Dependabot, Renovate, GitHub Actions and other apps. "Mute this repository" (in any repository's ⧉ menu) hides a repository's items everywhere; the sources panel lists muted repositories with their item counts to unmute them. "Hide archived" and "Hide dormant repos" drop items of archived repositories, which are then not even searched, and of dormant repositories: no commit on the default branch for 90+ days when logged in, nothing pushed to any branch for as long otherwise; such repositories carry an "archived" or "dormant" tag on cards, group headers and the Repos filter. Stats, facets and source counts follow every rule.
...
- **Actions when logged in**: 👍 upvote, comment (Markdown), add/remove labels, assign/unassign yourself, close as completed or not planned / reopen (with confirmation). Items of archived repositories are read-only.
...
- **Bulk actions** (logged in): a checkbox on every card (shift-click for a range) or "Select all N shown", then add/remove labels, assign/unassign yourself, or close (completed or not planned, optionally with a remembered comment first, handy to sweep dormant items) / reopen from a sticky bar. Items of archived repositories are skipped. Actions run item by item with a progress counter and a cancel button; failures are listed per item. Options that only fit part of the selection say how many items they apply to.
...
- **Resilience and diagnostics**: a rendering crash shows the error with Reload and "Reset cached items and reload" buttons instead of a blank page. Settings › Diagnostics lists the rate limit per API resource (core, search, GraphQL), the last errors with timestamps (kept across reloads), the cache size with per-source fetch stamps, and a one-click cache reset.
